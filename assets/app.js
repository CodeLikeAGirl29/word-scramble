...
function updateDisplay() {
  ...
  // Display mistake letters
  mistakesDisplay.innerText = mistakes.join(", ");

  // Add red X icons for each mistake
  const mistakeIcons = document.getElementById("mistakeIcons");
  mistakeIcons.innerHTML = ""; // Clear previous icons
  for (let i = 0; i < mistakes.length; i++) {
    const icon = document.createElement("i");
    icon.classList.add("fas", "fa-times");
    icon.style.color = "red";
    icon.style.marginLeft = "5px";
    mistakeIcons.appendChild(icon);
  }
}
